...
namespace LittleFairy_Race
{
    ...
    static class LitF_Harmony
    {
        static LitF_Harmony()
        {
            ...
            // 肉を動物扱いにする
            ThingDef meat = DefDatabase<ThingDef>.AllDefsListForReading.Where(x => x.defName == "Meat_LittleFairy_Pawn").First();
            meat.description = "MeatDesc".Translate(meat.label);
            meat.graphicData.texPath = "Things/Item/Resource/MeatFoodRaw/Meat_Small";
        }
    }
    ...
    {
        [HarmonyPrefix]
        static bool Prefix(ref float __result, Pawn pawn, StringBuilder sb)
        {
